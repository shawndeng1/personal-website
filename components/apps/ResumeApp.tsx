...
export function ResumeApp() {
  return (
    <div className="resume-app">
      ...
      <div className="resume-note"><b>PLACEHOLDER DATA</b> Replace bracketed content in the data folder and add your PDF to <code>public/resume.pdf</code>.</div>
      <div className="resume-body">
        <section><h3>Experience</h3>{experience.slice(0, 2).map((item) => <article className="resume-entry" key={item.company}><header><div><b>{item.role}</b><span>{item.company}</span></div><div><b>{item.dates}</b><span><MapPin />{item.location}</span></div></header><ul>{item.accomplishments.map((point) => <li key={point}>{point}</li>)}</ul></article>)}</section>
